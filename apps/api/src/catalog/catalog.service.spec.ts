...
describe('CatalogService', () => {
  ...
  it('createProduct validates, creates, reconciles image ref, bumps + audits', async () => {
    const tx = buildTx();
    const svc = build(tx);
    const input = { categoryId: 'cat_1', slug: 'prod-a', sortOrder: 0, title: { en: 'A', vi: 'A' }, imageId: 'img1' };
    ...
    expect(tx.assetRef.createMany).toHaveBeenCalledWith({
      data: [{ ownerType: 'product', ownerId: 'prod_1', field: 'image', assetId: 'img1', alt: undefined }],
    });
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'product.create', entityType: 'product', entityId: 'prod_1' }),
    });
  });
  ...
  it('deleteProduct soft-deletes (sets deletedAt) and bumps', async () => {
    ...
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'product.delete', entityType: 'product', entityId: 'prod_1' }),
    });
  });
  ...
  it('updateCategory validates, updates, reconciles image ref, bumps + audits', async () => {
    const tx = buildTx();
    const svc = build(tx);
    const input = { slug: 'pvc', sortOrder: 1, title: { en: 'PVC2', vi: 'PVC2' }, imageId: 'a2', imageAlt: { en: 'alt', vi: 'alt' } };
    const res = await svc.updateCategory({ id: 'u1' }, 'cat_1', input, 1);
    expect(res).toEqual({ revision: 2 });
    expect(shared.categoryInputSchema.parse).toHaveBeenCalledWith(input);
    expect(tx.category.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'cat_1' } }));
    expect(tx.assetRef.createMany).toHaveBeenCalledWith({
      data: [{ ownerType: 'category', ownerId: 'cat_1', field: 'image', assetId: 'a2', alt: { en: 'alt', vi: 'alt' } }],
    });
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'category.update', entityType: 'category', entityId: 'cat_1' }),
    });
  });
});

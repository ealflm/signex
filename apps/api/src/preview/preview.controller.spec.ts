...
describe('PreviewController.snapshot', () => {
  ...
  it('rejects an absent secret', async () => {
    await expect(controller.snapshot(undefined, 'ctheme-x')).rejects.toThrow(
      ForbiddenException,
    );
  ...
  it('rejects a wrong secret', async () => {
    await expect(controller.snapshot('nope', 'ctheme-x')).rejects.toThrow(
      ForbiddenException,
    );
  });

  it('returns the requested theme draftSnapshot (themeId from query)', async () => {
    const res = await controller.snapshot(SECRET, 'ctheme-x');
  ...
  it('accepts themeId from the body', async () => {
    await controller.snapshot(SECRET, undefined, { themeId: 'ctheme-body' });
    expect(prisma.client.theme.findUniqueOrThrow).toHaveBeenCalledWith({
  ...
  it('falls back to the live theme when themeId is omitted', async () => {
    const res = await controller.snapshot(SECRET);
  ...
  it('throws NotFound when themeId omitted and no live theme exists', async () => {
    ...
    await expect(controller.snapshot(SECRET)).rejects.toThrow(
      NotFoundException,
    );
  });
});

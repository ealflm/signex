...
- `globals.css`: `.hero-quote_panel .text_input-label { color: var(--sx-form-label, inherit) }` — the uniform colour; `inherit` keeps today's look bit-for-bit when unset. Per-label per-element overrides still win: the palette `<style>` (injected after the app stylesheet) targets the anchored span directly.
...
**Design:**
- **Size:** desktop `9.75rem` (3×); `@media (max-width: 767px)` → `4.875rem` (1.5×, per user decision). Icon/text scale proportionally (call SVG sized in %/em; Zalo wordmark font-size in em). Container offsets/gap adjusted so the stack clears the viewport edge.
